...
std::vector<std::vector<double>> mel_filterbank(int nfilt, int NFFT, int sr)
{
    ...
    {
        // Enorm : Normalisation d'aire (Slaney)
...
// --- MODIFICATION 3 : Intégration dans votre architecture temporelle exacte ---
std::vector<AudioFrame> mfcc_calc(const std::string &wav_path)
{
    ...
    // VOS paramètres d'origine, intouchés.
    ...
    // 128 filtres Mel (Défaut Librosa) pour avoir la même fidélité spectrale,
    // mais on garde 13 ceps à la fin.
    int nfilt = 128;
    int num_ceps = 13;
    ...
    // VOTRE calcul d'origine pour le nombre de frames
    ...
    // Passe 1 : Spectrogramme Mel + Max Global
    for (int i = 0; i < num_frames; i++)
    {
        // VOTRE boucle de copie d'origine (pas de padding artificiel)
...
void write_csv(const std::string &output_path, const std::vector<AudioFrame> &data, bool write_header = true)
{
    ...
    {
        file << "filename,frame_index,target_f0";
        for (int i = 0; i < 13; i++)
            file << ",mfcc_" << i;
        file << "\n";
    }
    ...
    {
        file << frame.filename << "," << frame.frame_index << ",";
        for (int i = 0; i < 13; i++)
            file << "," << frame.mfcc[i];
        file << "\n";
    }
